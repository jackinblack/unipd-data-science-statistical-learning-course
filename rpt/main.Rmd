...
We would like to treat the Hour predictor as a categorical variable in order to be able to capture non linear relationships with the target in our model as we will see in the next chapter.
...
```{r}
hist.variables = subset(data, select = c(Rented.Bike.Count, Temperature, Humidity, Wind.speed
	,Visibility, Dew.point.temperature, Solar.Radiation, Rainfall, Snowfall));
hist.variables$Hour <- Hour;
par(mfrow = c(3, 4));
