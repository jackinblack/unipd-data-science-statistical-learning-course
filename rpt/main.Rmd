...
```{r}
which(data$Humidity == 0);
```

## Data preprocessing

As our first step, we will select for our analysis only those hours where the system is functioning, since on the hours where it doesn't we are going to observe 0 counted rented bikes independently from the weather.
In such cases, this feature does not provide additional predictive power to the model and/or could cause overfitting. We also proceed by removing the feature entirely because we are only going to be using one of it's two categories.
...
We also need to transform our predictors Date and Hour into a date format correctly interpretable by R. We decide to remove the Date feature as our analysis will be focused on the predictive power of the weather related features and in addition the dataset only covers one solar year and the date would not be able to have a desired impact on prediction.
...
As described in the above paragraph, we proceed by replacing the incorrect humidity values with the average of the last and next strictly positive values.
For this purpose we use an *ad hoc* for loop that allows us to obtain the correct substitutions for our particular case.
```{r}
first = -1; first.idx = -1;
...
We would like to treat the Hour predictor as a categorical variable in order to be able to capture non linear relationships with the target in our modelling as we will see in the next chapter.
We will save this variable *as is* for future analysis.
```{r}
Hour <- data$Hour;
...
The main purpose of Exploratory data analysis (EDA) is to get a clear picture of the data before modelling through plots and summaries by identifying patterns and trends. It also allows to reveal unexpected relationships. Also, EDA helps find errors
and inconsistencies in the data.

In the next paragraphs we push to achieve the following:

- learn variable distributions and suitability for the poisson regression model
...
```{r}
hist.variables = cbind(
	subset(data, select = c(Rented.Bike.Count, Temperature, Dew.point.temperature, Solar.Radiation
		,Humidity, Visibility, Snowfall, Wind.speed, Rainfall)
	)
	,Hour = Hour);
par(mfrow = c(3, 4));
for (var in names(hist.variables)) {
	hist(hist.variables[[var]], right = FALSE, probability = TRUE
		,main = paste("Histogram of", var)
		,xlab = var, col = "skyblue", border = "white", cex.main = 0.8);
	lines(density(hist.variables[[var]]), col = "darkblue", lwd = 2);
	var.mean <- mean(hist.variables[[var]], na.rm = TRUE);
	var.median <- median(hist.variables[[var]], na.rm = TRUE);
	unique.out <- unique(hist.variables[[var]]);
	var.mode <- unique.out[which.max(tabulate(match(hist.variables[[var]], unique.out)))];
...
One assumption for the Poisson model is that the response variables should follow a Poisson distribution.
As seen from the plots the distribution of our target follows a Poisson distribution characterized by a small rate parameter ${\mu}$.
We can also observe that Temperature and Dew.point.temperature follow a bimodal distribution indicating the presence of two different subpopulations or groups 
within the data but this is expected since autumn and Spring have very similar temperatures and summer and Winter are opposites as we will see in the following paragraphs.

## Correlation matrix

In order to find linear relationships between our continuous predictors our first step is to build a correlation matrix.
...
The most important numerical predictors, in linear terms, for our target seem to be Temperature, Humidity and Dew point temperature. We can also
...
In the next paragraph, our objective is to investigate the most significant correlations with our target.
...
plot(timestamp, data$Rented.Bike.Count
...
lines(loess.smooth(timestamp, data$Rented.Bike.Count), lty = 1, col = 1);
```

Here we notice how bike share demand peaks in summer and declines in winter, this might imply that the bike
shares are high when the weather is warm. Although we are looking for a relationship between time and shares we 
observe that this relationship exists in function of weather temperature. Since no long-term relationship is visible 
between time and bike shares we don't expect to draw any knowledge from using time in our model.

```{r}
palette <- colorRampPalette(c("darkblue", "skyblue", "orange", "red", "darkblue"))(24);
plot(timestamp, data$Rented.Bike.Count
	,col = palette[Hour], pch = ifelse(data$Holiday == "No Holiday", 16, 3)
	,cex = 0.5, main = "Bike shares vs. Date"
	,xlab = "Date", ylab = "Rented Bike count");
...
Although the correlation between target and Hour is relatively significant, by inspecting this last scatter plot
...
The legend helps to further analyze seasonal patterns: winter and summer are positioned on opposite ends of the temperature spectrum, with winter assiciated to lower temperatures and bike shares and summer associated to higher temperatures and increased bike shares. Spring and autumn are clustered in the middle range of temperatures associated to moderate bike shares. This seasonal variation suggests that temperature and season significantly influence target behavior.

```{r}
palette <- colorRampPalette( c("#1D4E89", "#D0E1F9", "#1D4E89") )(24);
colors <- palette[Hour + 1]
plot(Hour, data$Rented.Bike.Count
	,col = colors
...
In this plot we can notice how the correlation between the target and Hour is explained by shares peak during rush hour this shows a conditional dependency.
Although overall the number of bike rentals varies with the seasons, one key feature remains consistent: peak demand times consistently occur at 8AM and 6PM.
...
```
Above we can see that humidity displays a uniform behavior across all seasons as we have highs and lows for all four seasons. However, it is notable that the number of bike shares tend to decrease as humidity levels rise, indicating a negative trend between high humidity and bike rental activity.
...
1. The data ${y = \{y_1, y_2, \ldots, y_n\}}$ are assumed to be independently distributed with means:
   $${\mu = \{\mu_1, \mu_2, \ldots, \mu_n\},\ with\ \mu_i > 0}$$ 
...
```{r}
model <- glm(Rented.Bike.Count ~., family = "poisson", data=data)
summary(model)
library(car)
vif(model)
```
...
abline(a = 0, b = 8.55, col = "red", lwd = 2);
```

We can also notice that some of the points lie at a large distance from the main cluster, these points could potentially be outliers.

## Model comparisons

To test whether our reduced model is more adequate to describe the data we employ a Chi-square test.
```{r}
anova(model, model1, test = "Chisq");
```

The deviance for the full model is lower than the deviance for the reduce model. Hence, we conclude that the full model fits the data better. The extremely significant p-value (< 2.2e-16) further supports that the inclusion of Dew.point.temperature provides a mild improvement to the model.
The difference in deviance between the two models is -10633, which indicates that the full model has a lower deviance than the reduced model. In the context of the deviance measure, a lower deviance suggests a better fit to the data.
In the presence of high VIF for Dew.point.temperature, indicating multicollinearity, it's not possible to conclude that the reduced model can be descarded a prori.  
