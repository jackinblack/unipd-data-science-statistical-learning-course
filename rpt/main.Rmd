...
```{r}
seoul.bike.data$Season[as.numeric(format(seoul.bike.data$Datetime, "%m")) %in% c(3, 4, 5)] <- "Spring";
seoul.bike.data$Season[as.numeric(format(seoul.bike.data$Datetime, "%m")) %in% c(6, 7, 8)] <- "Summer";
seoul.bike.data$Season[as.numeric(format(seoul.bike.data$Datetime, "%m")) %in% c(9, 10, 11)] <- "Autumn";
seoul.bike.data$Season[as.numeric(format(seoul.bike.data$Datetime, "%m")) %in% c(12, 1, 2)] <- "Winter";
season.color = c("Spring" = "green", "Summer" = "red", "Autumn" = "orange", "Winter" = "blue")
plot(
	seoul.bike.data$Datetime
	,seoul.bike.data$Rented.Bike.Count
	,col = season.color[seoul.bike.data$Season]
	,pch = 16
	,cex = 0.5
	,main = "Rented Bike count vs. Datetime"
	,xlab = "Datetime"
	,ylab = "Rented Bike count"
);
legend("topleft", legend = names(season.color), col = season.color
  ,pch = 16, title = "Season", cex = 0.7);
```

Here we notice how bike share demand peaks in summer 
