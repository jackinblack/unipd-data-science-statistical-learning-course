...
Notice how our Humidity predictor sometimes is reported as 0%, since this scenario is highly unlikely we
assume that these values are attributed to a malfunctioning of the devices responsible for this predictor. We
proceed by replacing these values with the median of the remaining values.

```{r}
median.humidity <- median(seoul.bike.data$Humidity[seoul.bike.data$Humidity != 0], na.rm = TRUE);
seoul.bike.data$Humidity[seoul.bike.data$Humidity == 0] <- median.humidity;
```
...
Our dataset contains the following categorical variables: Seasons, Holiday, Functional Day. As seen before R treats them as character variables but we need to transform them into factors for R to interpret them correctly.

```{r}
seoul.bike.data$Seasons 		<- as.factor( seoul.bike.data$Seasons );
seoul.bike.data$Holiday 		<- as.factor( seoul.bike.data$Holiday );
seoul.bike.data$Functioning.Day <- as.factor( seoul.bike.data$Functioning.Day );
```
...
par(mfrow = c(3, 4));
for (i in 2:11) {
	hist(seoul.bike.data[[i]], right = FALSE, probability = TRUE
	...
	unique.out <- unique(seoul.bike.data[[i]]);
	var.mode <- unique.out[which.max(tabulate(match(seoul.bike.data[[i]], unique.out)))];
...
plot(seoul.bike.data$tmstmp, seoul.bike.data$Rented.Bike.Count
	,col = palette[seoul.bike.data$Season], pch = 16
...
plot(seoul.bike.data$tmstmp, seoul.bike.data$Rented.Bike.Count
	,col = palette[seoul.bike.data$Hour], pch = ifelse(seoul.bike.data$Holiday == "No Holiday", 16, 3)
	,cex = 0.5, main = "Bike shares vs. Timestamp"
	,xlab = "Timestamp", ylab = "Rented Bike count");
...
```{r}
palette <- colorRampPalette( c("#6A3D9A", "#984EA3", "#7570B3", "#D95F0E", "#FF7F00", "#6A3D9A") )(24);
...
colors <- palette[seoul.bike.data$Hour + 1]
plot(seoul.bike.data$Hour, seoul.bike.data$Rented.Bike.Count
	,col = colors
	,xlab = "Hour", ylab = "Rented Bike count"
	,main = "Bike shares vs. Hour"
	,pch = 16, cex = 0.7);
lines(loess.smooth(seoul.bike.data$Hour, seoul.bike.data$Rented.Bike.Count), lty = 1, col = 1);
```
...
vif(model)
```
From the summary we can see that all the variables came out as significant. Since our exploratory data analysis made us suspect that collinearity might be present we also decided to run a Variance Inflation Factor analysis. From it we can see how temperature.C. and Dew.point.temperature..C. have a high VIF value. In our second model we decided to refit a poisson regression but without Dew.point.temperature..C. this time. Here's the result:
...
vif(model1)
```
We can see that by eliminating Dew.point.temperature..C. we see a significant improvement in the VIF results. The resulting model shows positive coefficient for Hour7 to Hour9, and for Hour15 to Hour23 with Hour24 as the benchmark factor. Temperature.C., Solar.Radiation..MJ.m2., and Holiday also exhibit a positive coefficient, meaning that the model will predict a higher value for the dependent variable as the covariate value increases, with a small p-value indicating high significance. All the other variables have a negative coefficients meaning that the model will predict a lower value for the dependent variable as the covariate value increases.
...
```{r}
mean_rented_bike_count <- mean(seoul.bike.data$Rented.Bike.Count)
var_rented_bike_count <- var(seoul.bike.data$Rented.Bike.Count)
...
future_data <- data.frame(
  Date = as.Date(c("2024-07-01", "2024-07-02", "2024-07-03")),  # Future dates
  Temperature = c(25, 26, 27),  # Example future temperatures
  Humidity = c(60, 65, 70),     # Example future humidity levels
  Wind.speed = c(2, 3, 2.5),     # Example future wind speeds
  Hour = factor(1),
  Visibility = 1000,
  ...
  # Add other predictors as necessary
)
future_data$Predicted.Bike.Count <- predict(model1, newdata = future_data, type = "response")
predict(model1, newdata = future_data, type = "response")
