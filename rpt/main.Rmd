...
Notice how our Humidity predictor sometimes is reported as 0%, since this scenario is highly unlikely we assume that these values 
are attributed to a malfunctioning of the devices responsible for this predictor. We proceed by replacing these values with the median
of the remaining values.

```{r}
# take previous correct value and average with the one right after 
median_humidity <- median(seoul.bike.data$Humidity[seoul.bike.data$Humidity != 0], na.rm = TRUE)
seoul.bike.data$Humidity[seoul.bike.data$Humidity == 0] <- median_humidity
```
...
threshold 				<- 1/5;
covariance.matrix 		<- var( seoul.bike.data[2:11], use="pairwise.complete.obs" );
...
plot(seoul.bike.data$tmstmp, seoul.bike.data$Rented.Bike.Count
	...
	,cex = 0.5, main = "Bike shares vs. Timestamp"
	,xlab = "Timestamp",ylab = "Rented Bike count" );
legend("topleft", legend = names(ssn.clr), col = ssn.clr
	,pch = 16, title = "Season", cex = 0.7);
lines(loess.smooth(seoul.bike.data$tmstmp, seoul.bike.data$Rented.Bike.Count), lty = 1,col = 1);
```
...
plot(seoul.bike.data$tmstmp, seoul.bike.data$Rented.Bike.Count
	,col = hr.clr[seoul.bike.data$Hour], pch = ifelse(seoul.bike.data$Holiday == "No Holiday", 16, 3)
	,cex = 0.5, main = "Bike shares vs. Timestamp"
	,xlab = "Timestamp", ylab = "Rented Bike count");
...
legend("topleft", ncol = 3, legend = seq(1,24), col = hr.clr
	,pch = 16, title = "Hour", cex = 0.7);
lines(loess.smooth(seoul.bike.data$Temperature, seoul.bike.data$Rented.Bike.Count), lty = 1,col = 3);
```
...
```{r}
par(mfrow = c(1, 2));
hday.clr = c("No holiday" = "#6A3D9A", "Holiday" = "#FF7F00");
plot(seoul.bike.data$Temperature, seoul.bike.data$Rented.Bike.Count
	,col = hday.clr[seoul.bike.data$Holiday], pch = 16
	,cex = 0.5, main = "Bike shares vs. Temperature"
	,xlab = "Temperature", ylab = "Rented Bike count");
legend("topleft", legend = names(hday.clr), col = hday.clr
	,pch = 16, title = "Holiday", cex = 0.7);
lines(loess.smooth(seoul.bike.data$Temperature, seoul.bike.data$Rented.Bike.Count), lty = 1, col = 1);
plot(seoul.bike.data$tmstmp, seoul.bike.data$Rented.Bike.Count
	,col = hday.clr[seoul.bike.data$Holiday], pch = 16
	,cex = 0.5, main = "Bike shares vs. Timestamp"
	,xlab = "Timestamp", ylab = "Rented Bike count");
...
```{r}
par(mfrow = c(2, 2));
wthr.clr = c("Sunny" = "#f7e8f8", "Moderate" = "#e3bce7", "Windy" = "#d090d6",
	"Cold" = "#bc63c5", "Hot" = "#a838b4", "Humid" = "#9306a3", "Wet" = "#7d0072");
boxplot(Rented.Bike.Count ~ weekdays(seoul.bike.data$tmstmp), data = seoul.bike.data
	...
	,main = "Bike shares vs. Day of the Week"
...
plot(seoul.bike.data$Humidity, seoul.bike.data$Rented.Bike.Count
	...
	,main = "Bike shares vs. Humidity"
	,col = wthr.clr[seoul.bike.data$Weather], pch = ifelse(seoul.bike.data$Holiday == "No Holiday", 16, 3)
	,cex = 0.7);
legend("topleft", ncol = 2, legend = names(wthr.clr), col = wthr.clr
	,pch = 16, title = "Weather cond.", cex = 0.4);
lines(loess.smooth(seoul.bike.data$Humidity, seoul.bike.data$Rented.Bike.Count), lty = 1,col = 1);
plot(seoul.bike.data$Wind.speed, seoul.bike.data$Rented.Bike.Count
	,xlab = "Wind Speed (m/s)", ylab = "Rented Bike count"
	,main = "Bike shares vs. Wind Speed"
	,col = wthr.clr[seoul.bike.data$Weather], pch = ifelse(seoul.bike.data$Holiday == "No Holiday", 16, 3)
	,cex = 0.7);
legend("topright", ncol = 2, legend = names(wthr.clr), col = wthr.clr
	,pch = 16, title = "Weather cond.", cex = 0.4);
lines(loess.smooth(seoul.bike.data$Wind.speed, seoul.bike.data$Rented.Bike.Count), lty = 1,col = 1);
plot(seoul.bike.data$Hour, seoul.bike.data$Rented.Bike.Count
	,xlab = "Hour", ylab = "Rented Bike count"
	,main = "Bike shares vs. Hour"
	,col = "skyblue", pch = 16, cex = 0.7);
```


```{r}
is.factor(seoul.bike.data$Holiday)
boxplot(log(seoul.bike.data$Rented.Bike.Count) ~ weekdays(seoul.bike.data$tmstmp))
boxplot((seoul.bike.data$Rented.Bike.Count) ~ (seoul.bike.data$Holiday))
bartlett.test((seoul.bike.data$Rented.Bike.Count) ~ weekdays(seoul.bike.data$tmstmp))
bartlett.test((seoul.bike.data$Rented.Bike.Count) ~ (seoul.bike.data$Holiday))
anova_result <-aov((seoul.bike.data$Rented.Bike.Count) ~ weekdays(seoul.bike.data$tmstmp))
summary(anova_result)
anova_result <-aov((seoul.bike.data$Rented.Bike.Count) ~ (seoul.bike.data$Holiday))
summary(anova_result)
anova_result <- aov(Rented.Bike.Count ~ Seasons, data = seoul.bike.data)
summary(anova_result)
...
plot(log(seoul.bike.data$WeatherScore), seoul.bike.data$Rented.Bike.Count
	...
	,cex = 0.5, main = "Bike shares vs. WeatherScore"
...
predictors <- c("Temperature", "Humidity", "Wind.speed", "Visibility", "Solar.Radiation", "Rainfall", "Snowfall")
response <- "Rented.Bike.Count"

# Create a matrix of predictors
X <- as.matrix(seoul.bike.data[, predictors])

# Create a vector of the response variable
y <- seoul.bike.data[, response]
X <- scale(X)

# Split the data into training and testing sets
set.seed(123) # for reproducibility
train_index <- sample(1:nrow(seoul.bike.data), 0.95 * nrow(seoul.bike.data)) # 70% for training
train_data <- seoul.bike.data[train_index, ]
test_data <- seoul.bike.data[-train_index, ]

# Train the Lasso model
lasso_model <- glmnet(x = X[train_index, ], y = y[train_index], alpha = 1)

# Tune the Lasso model using cross-validation
cv_lasso <- cv.glmnet(x = X[train_index, ], y = y[train_index], alpha = 1)
#lasso_pred <- predict(lasso_model, newx = X[-train_index, ], s = "lambda.min")
optimal_lambda <- cv_lasso$lambda.min
lasso_pred <- predict(lasso_model, newx = X[-train_index, ], s = optimal_lambda)

# Calculate the mean squared error
mse <- mean((lasso_pred - y[-train_index])^2)

# Calculate the total sum of squares (TSS)
tss <- sum((y[-train_index] - mean(y[-train_index]))^2)
...
abline(v=log(lasso_model$lambda[50]), lty=3, lwd=2)

# Evaluate the model performance on the testing data
cor(seoul.bike.data$Rented.Bike.Count, (seoul.bike.data$WeatherScore)^1/3)
```

## Distribution

The ﬁrst step in most data analysis applications is the exploration of the statistical properties of the variables. This is typically
achieved by plotting the probability densities, which succinctly summarize each variable for visualization. One way to obtain an
empirical non-parametric density estimate is by using histograms. Although histograms are considered crude for most advanced statistical
applications, they have the great advantage of making no prior assumptions regarding the distribution of the examined variable
and are very simple to compute. Often, this preliminary step can reveal whether the variable follows a Gaussian (normal) distribution,
which is characterized by a unimodal peak in the middle of the variables possible range of values, is completely symmetric,
and is particularly useful because a large number of mathematical functions are applicable.

```{r}
par(mfrow = c(3, 4));
for (i in 2:11) {
	hist(seoul.bike.data[[i]], breaks = 30, probability = TRUE
		,main = paste("Histogram of", names(seoul.bike.data)[i])
		,xlab = names(seoul.bike.data)[i], col = "skyblue", border = "white", cex.main = 0.8);
	lines(density(seoul.bike.data[[i]]), col = "darkblue", lwd = 2);
	var.mean <- mean(seoul.bike.data[[i]], na.rm = TRUE);
	var.median <- median(seoul.bike.data[[i]], na.rm = TRUE);
	unique.res <- unique(seoul.bike.data[[i]])
	var.mode <- unique.res[which.max(tabulate(match(seoul.bike.data[[i]], unique.res)))]
